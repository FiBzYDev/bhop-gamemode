CreateConVar("kawaii_showtrailling", "0", FCVAR_ARCHIVE, "Set to 1 to show trailing, 0 to hide")

if GetConVar("kawaii_showtrailling"):GetInt() ~= 1 then
    return
end

local st, lp, Iv, ey, ler, con, gt, near, col, cw, cd, rat, fr, lst = SysTime, LocalPlayer, IsValid, EyeAngles, Lerp, Timer:GetConvert(), Timer.GetTimeDifference, { pos = Vector() }, { r = 255, g = 0, b = 0, a = 80 }, Color( 255, 255, 255 ), Color( 40, 40, 40 ), 100, 150
...
function ENT:LoadConfig()
	self.blue = Client:GetTrailConfig "blue"
	self.range = Client:GetTrailConfig "range"
	self.ground = Client:GetTrailConfig "ground"
	self.alpha = Client:GetTrailConfig "vague"
	self.nolandmark = Client:GetTrailConfig "label"
	self.nohud = Client:GetTrailConfig "hud"
end
