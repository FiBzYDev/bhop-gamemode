...
local mapDatabase = {
    ["bhop_overline"] = "00:23:049 - bozya", ["bhop_stref_amazon"] = "00:19:140 - astra",
    ["bhop_asko"] = "02:02:320 - bozya", ["bhop_anotherbhopmap"] = "01:10.829 - Cat",
    ["bhop_lubluebatsjavanal"] = "00:34.309 - bozya", ["bhop_effigy"] = "00:12.480 - Vehnex",
    ["bhop_horseshit_4"] = "00:16.199 - Justa", ["bhop_terry"] = "01:00.999 - Vehnex",
    ["bhop_kotodama"] = "00:41.029 - zzz", ["bhop_kotodama2"] = "00:45.679 - justa",
    ["bhop_bloodflow"] = "00:41.410 - Vehnex", ["bhop_speedrun_valley"] = "00:18.529 - Vehnex",
    ["bhop_p08"] = "00:36.850 - Greatchar", ["bhop_3dm"] = "00:31.439 - justa",
    ["bhop_newdun"] = "00:21.670 - Vehnex", ["bhop_filter_fix"] = "00:17.119 - Justa",
    ["bhop_nosotros"] = "00:26.378 - Justa", ["bhop_kiwi_cwfx"] = "00:21.739 - zzz",
    ["bhop_linear_gif2"] = "00:39.938 - Lemin", ["bhop_bfur"] = "00:09.531 - Sashbern",
    ["bhop_sandtrap3"] = "00:49.948 - Lemin", ["bhop_nervosity2"] = "00:39.740 - Cat",
    ["bhop_coast"] = "00:33.569 - Greatchar", ["bhop_stref_lively"] = "00:18.489 - LokL",
    ["bhop_hexag0n_cool"] = "00:25.499 - Cat", ["bhop_blossom"] = "00:37.729 - bozya",
    ["bhop_futile"] = "00:51.489 - Lemin", ["bhop_bloc"] = "00:54.091 - Cat",
    ["bhop_bon_fix"] = "00:20.117 - justa", ["bhop_rikudo"] = "00:25.540 - justa",
    ["bhop_akame"] = "00:21.678 - justa", ["bhop_lined"] = "00:27.329 - LokL",
    ["bhop_autobadges"] = "00:17.819 - antacid", ["bhop_ragepoop_rev"] = "00:17.040 - boyza",
    ["bhop_probeginners"] = "00:18.190 - m1ners", ["bhop_chaser"] = "00:19.369 - Vehnex",
    ["bhop_jegg"] = "00:07.399 - Vehnex", ["bhop_blue_aux"] = "00:18.279 - Greatchar",
    ["bhop_zhopavqaqe"] = "00:07.709 - Cat", ["bhop_z"] = "00:59.738 - justa",
    ["bhop_dimensional"] = "01:13.690 - Trickster", ["bhop_zeus"] = "00:58.209 - Lemin",
    ["bhop_saffron_css"] = "00:18.708 - LokL", ["bhop_grassyass"] = "00:14.309 - astra",
    ["bhop_ambience"] = "00:17.919 - LokL", ["bhop_cobblestone"] = "00:26.880 - dora",
    ["bhop_bhop"] = "00:29.519 - Syn", ["bhop_bhop3"] = "00:31.910 - Sashbern",
    ["bhop_topgay"] = "00:26.060 - Sashbern", ["bhop_alt_saimaa"] = "00:19.977 - Cat",
    ["bhop_alt_univaje"] = "00:21.430 - Zqk", ["bhop_kasvihuone"] = "00:37.929 - sadrainbow",
    ["bhop_stref_siberia"] = "00:27.910 - Zqk", ["bhop_horseshit_1"] = "00:25.299 - Sashbern"
}
...
local function DisplayKeys()
    ...
    if isValidSpectator then
        for key, text in pairs(keyStrings) do
            ...
            if not willDisplay then continue end
            local pos = keyPositions[currentPos][key]
...
local function pGOldTime(input)
    ...
    local s = math.floor(input % 60)
    local t = math.floor((input * 10) % 10)
...
HUD.Themes = {
	...
	function(pl, data)
		...
		if not (lp():Team() == TEAM_SPECTATOR) then 
			if type(sync) == 'number' then 
				...
				if sync ~= 0 then
					...
					col = sync < 90 and sd or col
					col = sync == 0 and color_white or col
					local formattedSync = string.format("%.2f", sync)
					draw.SimpleText(formattedSync, "HUDTimerKindaUltraBig", ScrW() / 2, yPos + height + 34, col, TEXT_ALIGN_CENTER)
				end
				...
				if roundedBoxEnabled:GetBool() then
					draw.RoundedBox(8, xPos - 10, ScrH() - 24, barWidth, 16, col)
				else
		...
		local start = plyPos + Vector(0, 0, 3)
		local endpos = plyPos - Vector(0, 0, 7)
	...
	function(pl, data)
		...
		local BASE = Color(35, 35, 35)
		local INNER = Color(42, 42, 42) 
	...
	function(pl, data)
		...
		local syncValue = data.sync or 0

		draw.SimpleText("Sync: " .. syncValue .. "%", "BottomHUDVelocity", 1895, posYz + 32, color_black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		draw.SimpleText("Sync: " .. syncValue .. "%", "BottomHUDVelocity", 1895, posYz + 30, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		...
		if velocity > 0 then
			local alpha = math.Clamp(255 * (velocity / 250), 0, 255)
		
			draw.SimpleText("Velocity: " .. velocity .. " u/s", "BottomHUDVelocity", (ScrW() / 2) + 2, posYz + 30 + 2, Color(0, 0, 0, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			draw.SimpleText("Velocity: " .. velocity .. " u/s", "BottomHUDVelocity", (ScrW() / 2) + 2, posYz + 30, Color(255, 255, 255, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	end,
	...
	function(pl, data)
		...
		if data.strafe then 
			...
			draw.SimpleText("Sync: " .. data.sync .. "%","HUDTimer", x, y + 23, TEXT, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end
...
local JHudStatistics = {0, 0, 0, 0, 0}
...
net.Receive("kawaii.secret", function(_, _)
    ...
    local lastspeed = net.ReadInt(18)

    JHudAnnounced = CurTime()
    JHudStatistics = {jumps, gain, speed, jss, lastspeed}
end)
...
local fade = 0
local onSlope = false
local slopeEndTime = 0

local function updateColorAlpha(color, fade)
    color.a = math.Clamp(color.a - fade, 0, 255)
end
...
local function drawCrosshair(gain)
    local gaincrosshair = GetConVarNumber("kawaii_gaincrosshair")
    if gaincrosshair == 1 then
        local colorCR = getColorForGain(gain)
        surface.DrawCircle(ScrW() / 2, (ScrH() / 2) + 0.5, 4, colorCR)
        surface.DrawCircle(ScrW() / 2, (ScrH() / 2) + 0.5, 3, colorCR)
    end
end

local function checkSlope(pl)
    local start = pl:GetPos() + Vector(0, 0, 10)
    local endpos = start + Vector(0, 0, -20)
    ...
    if tr.Hit and tr.HitNormal.z < 0.97 then
        onSlope = true
        slopeEndTime = CurTime() + 1
    else
        onSlope = false
    end
end

local function drawSpeed(speed, color, pl)
    if onSlope or CurTime() <= slopeEndTime then
...
local function SSJ_HUD()
    local jump, gain, speed, jss, lastspeed = unpack(JHudStatistics)
    local baseColor = getColorForGain(gain)
    local speedColor = getColorForSpeed(speed)

    if JHudAnnounced + 2 < CurTime() then
        fade = fade + 0.5
        updateColorAlpha(speedColor, fade)
        updateColorAlpha(baseColor, fade)
    else
        fade = 0
    end
    ...
    if jump <= 2 then
        drawSpeed(speed, speedColor, pl)
    else
        drawSpeed(speed, baseColor, pl)
    end
    ...
    if kawaiihud == 3 and jump <= 1 then
        draw.SimpleText(math.Round(speed, 0), "Simplefont", ScrW() / 2, yPos + 123, speedColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end

    if jump > 2 then
        local color_white_fade = Color(255, 255, 255, 255 - fade * 2)
        updateColorAlpha(color_white_fade, fade)
        draw.SimpleText(math.Round(lastspeed, 0), "JHUD1MainBIG2", ScrW() / 2, ScrH() / 2 - 140, color_white_fade, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText(math.Round(gain, 2) .. "%", "JHUD1MainBIG2", ScrW() / 2, ScrH() / 2 - 60, color_white_fade, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end

    if JHUD1OLD and kawaiihud == 4 then
        draw.SimpleText(math.Round(speed, 0), "JHUD1Main", ScrW() / 2, ScrH() / 2 + 291, speedColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        if jump > 2 then
            draw.SimpleText("(with " .. math.Round(gain, 2) .. "% gain)", "JHUD1Main", ScrW() / 2, ScrH() / 2 + 312, baseColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    end
end
