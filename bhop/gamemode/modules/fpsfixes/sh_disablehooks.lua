AddCSLuaFile("modules/fpsfixes/sh_disablehooks.lua")
...
-- Define a function to check if a global key is non-essential
...
-- Function to clear specific entries from a table
...
-- Function to remove specific hooks
local function RemoveSpecificHooks()
    local hooksToRemove = {
        ["SetupMove"] = "SetupMove",
        -- Add other hooks you want to remove here
    }

    for hookName, identifier in pairs(hooksToRemove) do
...
hook.Add("InitPostEntity", "DisablePhysicsSettings", function()
    local phys_settings = physenv.GetPerformanceSettings()

    phys_settings.LookAheadTimeObjectsVsObject = 0
    phys_settings.LookAheadTimeObjectsVsWorld = 0
    phys_settings.MaxAngularVelocity = 0
    phys_settings.MaxCollisionChecksPerTimestep = 0
    phys_settings.MaxCollisionsPerObjectPerTimestep = 0
    phys_settings.MaxFrictionMass = 0
    phys_settings.MaxVelocity = 0
    phys_settings.MinFrictionMass = 0
...
function GM:OnUndo() return end
function GM:CreateMove( ply, mv, cmd ) return end
function GM:FinishMove( ply, mv, cmd ) return end
function GM:StartMove( ply, mv, cmd ) return end
function GM:StartCommand() return end
function GM:EntityEmitSound() return end
function GM:PropBreak() return end
function GM:CanTool() return end
function GM:ShouldCollide() return true end
function GM:GravGunPunt() return end
function GM:UpdateAnimation() return end
function GM:HandlePlayerJumping() return end
function GM:PlayerTraceAttack( ply, dmginfo, dir, trace ) return end
function GM:SetPlayerSpeed( ply, walk, run ) return end
function GM:FindUseEntity( ply, ent ) return end
function GM:PlayerSwitchWeapon( ply, oldwep, newwep ) return end