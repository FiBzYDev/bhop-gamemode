...
function Trailing.MainCommand(ply, args)
    if not args or (#args == 0) then
        Core:Send(ply, "Print", {"Timer", "You must provide a Style name or a Style ID to use it."})
        return
    end
    ...
    if (textFormat == "hide") then
        ...
        Core:Send(ply, "Print", {"Timer", "All Bot trails have been hidden."})
        return
    end

    local styleID = tonumber(args[1])
    local convertedArgs = table.concat(args, " ")
    if styleID and not Core:IsValidStyle(styleID) then
        return Core:Send(ply, "Print", {"Timer", "The provided Style ID is not valid, please provide a valid Style ID or name to use it."})
    elseif convertedArgs and not Core:GetStyleID(convertedArgs) then
        return Core:Send(ply, "Print", {"Timer", "The provided Style name is not valid, please provide a valid Style ID or name to use it."})
    end

    if not styleID and convertedArgs then
        styleID = Core:GetStyleID(convertedArgs)
    end

    local res = Trailing.CreateOnStyle(ply, styleID)
    if (res == 0) then
        Core:Send(ply, "Print", {"Timer", "The trail for Bot " .. Core:StyleName(styleID) .. " has been generated and is now displayed."})
    elseif (res == 1) then
        Core:Send(ply, "Print", {"Timer", "The trail for Bot " .. Core:StyleName(styleID) .. " has been displayed."})
    else
        Core:Send(ply, "Print", {"Timer", "There does not seem to be any replay or trail available for this style, please try again later."})
    end
end
