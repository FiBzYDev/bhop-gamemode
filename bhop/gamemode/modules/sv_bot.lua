...
function Bot:Spawn(bMulti, nStyle, bNone)
	...
	for _, bot in pairs(player.GetBots()) do
		if bot.Temporary then
			bot:SetMoveType(MOVETYPE_NONE) -- Set the bot's move type to prevent physics interactions
			bot:SetCollisionGroup(COLLISION_GROUP_DEBRIS) -- Set collision group to avoid collisions with other entities
			bot:SetFOV( 100, 0 )
			bot.Style = nStyle
			bot.Temporary = nil
