...
Admin.DefaultSID = ""
Admin.GamemodeKey = "2"
Admin.Level = {None = 0,Base = 1,Elevated = 2, Moderator = 4,Admin = 8,ZAdmin = 9,Super = 16,Developer = 32,Manager = 33,Founder = 34,Owner = 64}
Admin.Icons = {[Admin.Level.Base] = 1,[Admin.Level.Elevated] = 2,[Admin.Level.Moderator] = 3,[Admin.Level.Admin] = 4,[Admin.Level.ZAdmin] = 5,[Admin.Level.Super] = 6,[Admin.Level.Developer] = 7,[Admin.Level.Manager] = 8,[Admin.Level.Founder] = 9,[Admin.Level.Owner] = 10}
...
Secure.SteamAPIKey = ""
Secure.Levels = {}
Secure.Setup = {{ 13, "Toggle Mute", Admin.Level.Elevated, { 390, 42, 100, 25, true } },{ 14, "Toggle Gag", Admin.Level.Elevated, { 495, 42, 100, 25, true } },{ 12, "Move to Spectator", Admin.Level.Admin, { 390, 77, 100, 25, true } },{ 23, "Strip weapons", Admin.Level.Admin, { 495, 77, 100, 25, true } },{ 4, "Monitor sync", Admin.Level.Admin, { 600, 77, 100, 25, true } },{ 15, "Kick Player", Admin.Level.Admin, { 390, 112, 100, 25, true } },{ 16, "Ban Player", Admin.Level.Admin, { 495, 112, 100, 25, true } },{ 25, "Unban Player", Admin.Level.Admin, { 600, 112, 100, 25, true } },{ 5, "Change Map", Admin.Level.ZAdmin, { 390, 167, 100, 25 } },{ 3, "Set Main Points", Admin.Level.ZAdmin, { 495, 167, 100, 25 } },{ 21, "Set Bonus Points", Admin.Level.ZAdmin, { 600, 167, 100, 25 } },{ 11, "Set Map Options", Admin.Level.ZAdmin, { 705, 167, 100, 25 } },{ 1, "Set Zone", Admin.Level.ZAdmin, { 390, 202, 100, 25 } },{ 10, "Remove Zone", Admin.Level.ZAdmin, { 495, 202, 100, 25 } },{ 2, "Cancel Creation", Admin.Level.ZAdmin, { 600, 202, 100, 25 } },{ 6, "Reload Zones", Admin.Level.ZAdmin, { 705, 202, 100, 25 } },{ 9, "Set Zone Height", Admin.Level.ZAdmin, { 390, 237, 100, 25 } },{ 19, "Set Bot Frame", Admin.Level.ZAdmin, { 495, 237, 100, 25 } },{ 20, "Cancel map vote", Admin.Level.ZAdmin, { 600, 237, 100, 25 } },{ 31, "Request Screen", Admin.Level.ZAdmin, { 705, 237, 100, 25, true } },{ 17, "Remove Time", Admin.Level.ZAdmin, { 390, 292, 100, 25 } },{ 18, "Remove Bot", Admin.Level.ZAdmin, { 495, 292, 100, 25 } },{ 28, "Remove all times for specific style", Admin.Level.Developer, { 600, 292, 205, 25 } },{ 24, "Reload Admins", Admin.Level.Developer, { 390, 327, 100, 25 } },{ 22, "Remove Map", Admin.Level.Developer, { 495, 327, 100, 25 } },{ 7, "Set Authority", Admin.Level.Manager, { 600, 327, 100, 25, true } },{ 8, "Remove Authority", Admin.Level.Manager, { 705, 327, 100, 25, true } },{ 29, "Send Notification", Admin.Level.Developer, { 390, 362, 100, 25, true } },{ 30, "Teleport Player", Admin.Level.ZAdmin, { 495, 362, 100, 25, true } },{ 27, "Incognito Spectating", Admin.Level.Admin, { 390, 402, 120, 25 } },{ 32, "Incognito Admin", Admin.Level.ZAdmin, { 515, 402, 120, 25 } },{ 26, "Legit Speed Zone", Admin.Level.ZAdmin },}
...
function Admin:CheckVIP( ply, nTime )
	SQL:Prepare(
		...
		if Core:Assert( data, "nID" ) then
			local item = data[ 1 ]
			local nLength = tonumber(item["nLength"])
			local length = (nLength or 0) * 60
			local remain
	
			if (not nLength) then 
				Admin:SetVIP( ply, item["nType"], item["szTag"], item["szName"], item["szChat"], 0, item["nID"] )
				return
			end
...
function Admin:AddNotification( szText )
	Core:Broadcast( "Print", { _C["Server"], szText } )
end
...
function Admin:SetVIPData( ply, szMessage, nType, szData )
	...
	SQL:Prepare(
		...
		if data then
			...
			Core:Send( varArg[ 1 ], "Print", { "VIP", "Nous n'avons pas pu enregistrer vos modifications. Veuillez réessayer." } )
		end
	end, { ply, szMessage } )
end
...
function Admin:VIPPanelCall( ply, args )
	...
	if szType == "Rainbow" and ply.VIPLevel and ply.VIPLevel >= Admin.Level.Elevated then
		...
		if szInvalid then
			return Core:Send( ply, "Print", { "VIP", "Vous aviez des morceaux de texte interdits en votre nom: " .. szInvalid } )
		end
		...
		Admin:SetVIPData( ply, "Votre nom est maintenant " .. (ply.VIPName[ 1 ].x > 255 and "" or "plus ") .. "affiché dans des couleurs arc-en-ciel", 2, string.Implode( " ", { math.floor( ply.VIPName[ 1 ].x ), math.floor( ply.VIPName[ 1 ].y ), math.floor( ply.VIPName[ 1 ].z ), ply.VIPName[ 2 ] } ) )
	elseif szType == "Gradient" and ply.VIPLevel and ply.VIPLevel >= Admin.Level.Elevated then
		if not ply.VIPSetGradient then
			...
			if szLen < 3 or szLen > 26 then
				return Core:Send( ply, "Print", { "VIP", "La longueur de votre nom doit être comprise entre 3 et 26 caractères" } )
			end
			...
			Admin:SetVIPData( ply, "Votre nom est maintenant affiché en dégradé", 2, string.Implode( " ", { math.floor( ply.VIPGradient[ 1 ].x ), math.floor( ply.VIPGradient[ 1 ].y ), math.floor( ply.VIPGradient[ 1 ].z ), math.floor( ply.VIPGradient[ 2 ].x ), math.floor( ply.VIPGradient[ 2 ].y ), math.floor( ply.VIPGradient[ 2 ].z ), ply.VIPName[ 2 ] } ) )
		end
	elseif szType == "Save" and ply.VIPLevel and ply.VIPLevel >= Admin.Level.Elevated then
		if data[ 2 ] then
			...
			Admin:SetVIPData( ply, "La couleur du texte de votre chat a été modifiée", 3, string.Implode( " ", { math.floor( ply.VIPChat.x ), math.floor( ply.VIPChat.y ), math.floor( ply.VIPChat.z ) } ) )
		end
	elseif szType == "Tag" then
		if data[ 3 ] == "" or data[ 3 ] == " " then
			...
			Admin:SetVIPData( ply, "Votre tag a été réinitialisé", 1, "" )
		else
			local szInvalid = Admin:IsVIPCustomValid( ply, data[ 3 ] )
			if szInvalid then
				return Core:Send( ply, "Print", { "VIP", "Vous aviez des morceaux de texte interdits en votre tag: " .. szInvalid } )
			end
			
			local szLen = string.len( data[ 3 ] )
			if szLen < 3 or szLen > 20 then
				return Core:Send( ply, "Print", { "VIP", "La longueur de votre tag doit être comprise entre 3 et 20 caractères" } )
			end
			...
			Admin:SetVIPData( ply, "Votre tag a été remplacé par: " .. ply.VIPTag[ 2 ], 1, string.Implode( " ", { math.floor( ply.VIPTag[ 1 ].x ), math.floor( ply.VIPTag[ 1 ].y ), math.floor( ply.VIPTag[ 1 ].z ), ply.VIPTag[ 2 ] } ) )
		end
	elseif szType == "Name" then
		if data[ 3 ] == "" or data[ 3 ] == " " then
			...
			Admin:SetVIPData( ply, "Votre nom a été réinitialisé: " .. ply:Name(), 2, "" )
		else
			local szInvalid = Admin:IsVIPCustomValid( ply, data[ 3 ] )
			if szInvalid then
				return Core:Send( ply, "Print", { "VIP", "Vous aviez des morceaux de texte interdits en votre nom: " .. szInvalid } )
			...
			Admin:SetVIPData( ply, "Votre nom a été changé en: " .. ply.VIPName[ 2 ], 2, string.Implode( " ", { math.floor( ply.VIPName[ 1 ].x ), math.floor( ply.VIPName[ 1 ].y ), math.floor( ply.VIPName[ 1 ].z ), ply.VIPName[ 2 ] } ) )
		end
	end
end
...
function Admin.VIPProcess( ply, args, extra )
	...
	if #args > 0 then
		...
		if szType == "remaining" then
			Core:Send( ply, "Print", { "VIP", "VIP Expire le: " .. ply.VIPExpire .. " - Minutes restantes: " .. (ply.VIPRemaining or 0) } )
		...
		elseif szType == "me" or szType == "emote" then
			if ply.VIPLevel >= Admin.Level.Elevated then
				...
				if tab and #tab > 0 then
					...
					Core:Send( ply, "Print", { "VIP", "Vous devez écrire un message à envoyer. Exemple: /me a une grosse queue" } )
				end
			else
				Core:Send( ply, "Print", { "VIP", "Cette commande n'est disponible que pour Elevated VIPs" } )
			end
		elseif szType == "gag" or szType == "mute" then
			...
			Core:Send( ply, "Print", { "VIP", "Commande non valide. Commandes VIP disponibles: remaining, extend, me, gag, mute" } )
		end
	else
		local tab = {
		...
		table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 20, 40 }, ["SetSize"] = { 200, 16 }, ["SetText"] = { "Normal VIP (Jours restants: " .. (ply.VIPRemaining == 0 and "Infini" or math.ceil( ply.VIPRemaining / 1440 )) .. ")" } } } )
		table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 20, 60 }, ["SetSize"] = { 200, 16 }, ["SetText"] = { "Tag couleur" } } } )
		...
		table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 20, 209 }, ["SetSize"] = { 60, 16 }, ["SetText"] = { "Tag texte:" } } } )
		table.insert( tab, { Type = "DTextEntry", Label = "TextTag", Modifications = { ["SetPos"] = { 75, 207 }, ["SetSize"] = { 145, 22 }, ["SetText"] = { tag } } } )
		
		table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 20, 242 }, ["SetSize"] = { 200, 16 }, ["SetText"] = { "Couleur nom" } } } )
		...
		table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 20, 389 }, ["SetSize"] = { 60, 16 }, ["SetText"] = { "Nom texte:" } } } )
		table.insert( tab, { Type = "DTextEntry", Label = "TextName", Modifications = { ["SetPos"] = { 85, 387 }, ["SetSize"] = { 135, 22 }, ["SetText"] = { name } } } )
		table.insert( tab, { Type = "DButton", VIP = true, Extra = "Tag", Modifications = { ["SetPos"] = { 20, 422 }, ["SetSize"] = { 95, 25 }, ["SetText"] = { "Sauvegarde Tag" } } } )
		table.insert( tab, { Type = "DButton", VIP = true, Extra = "Name", Modifications = { ["SetPos"] = { 125, 422 }, ["SetSize"] = { 95, 25 }, ["SetText"] = { "Sauvegarde Nom" } } } )
		
		if ply.VIPLevel >= Admin.Level.Elevated then
			tab.Width = tab.Width + 235
			
			table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 250, 40 }, ["SetSize"] = { 200, 16 }, ["SetText"] = { "Extended VIP (Jours restants: " .. (ply.VIPRemaining == 0 and "Infini" or math.ceil( ply.VIPRemaining / 1440 )) .. ")" } } } )
			
			table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 250, 60 }, ["SetSize"] = { 200, 16 }, ["SetText"] = { "Chat texte couleur" } } } )
			...
			table.insert( tab, { Type = "DLabel", Modifications = { ["SetPos"] = { 250, 365 }, ["SetSize"] = { 200, 100 }, ["SetText"] = { "Si vous voulez réinitialiser\nvotre tag ou nom, laissez juste la\ncase blanche et sauvegardez !" } } } )
			
			table.insert( tab, { Type = "DButton", VIP = true, Extra = "Random", Modifications = { ["SetPos"] = { 250, 207 }, ["SetSize"] = { 95, 25 }, ["SetText"] = { "Couleur aléatoire" } } } )
			table.insert( tab, { Type = "DButton", VIP = true, Extra = "Save", Modifications = { ["SetPos"] = { 355, 207 }, ["SetSize"] = { 95, 25 }, ["SetText"] = { "Sauvegarde chat" } } } )
			table.insert( tab, { Type = "DButton", VIP = true, Extra = "Rainbow", Modifications = { ["SetPos"] = { 250, 260 }, ["SetSize"] = { 200, 25 }, ["SetText"] = { "Activer couleur arc-en-ciel sur le nom" } } } )
			table.insert( tab, { Type = "DButton", VIP = true, Extra = "Gradient", Modifications = { ["SetPos"] = { 250, 295 }, ["SetSize"] = { 200, 25 }, ["SetText"] = { "Définir le dégradé (Clique pour aide)" } } } )
		end
...
function Admin:CreateWindow( ply )
	...
	local tab = {Title = "Timer Admin Panel",Width = 720,Height = 447,}
...
function Admin:HandleButton( ply, args )
	...
	elseif ID == 3 then
		local tabRequest = Admin:GenerateRequest( "Enter the map Points.", "Map multiplier", tostring( Timer.Multiplier ), ID )
	...
	elseif ID == 5 then
		...
		local currentMapWithoutPrefix = currentMap:gsub("^bhop_", "") -- Remove "bhop_" prefix from the current map name
		
		local tabRequest = Admin:GenerateRequest("Enter the map you want to change to", "Change map", currentMapWithoutPrefix, ID)
		Core:Send( ply, "Admin", { "Request", tabRequest } )
	elseif ID == 6 then
		Zones:Reload()
	...
	elseif ID == 9 then
		...
		for _,zone in pairs( Zones.Entities ) do
			if IsValid( zone ) then
				table.insert( tabQuery, { Zones:GetName( zone.zonetype ), { ID, zone:EntIndex() } } )
			end
		end
	...
	elseif ID == 10 then
		...
		for _,zone in pairs( Zones.Entities ) do
			if IsValid( zone ) then
				...
				table.insert( tabQuery, { Zones:GetName( zone.zonetype ) .. extra, { ID, zone:EntIndex() } } )
			end
		end
	...
	elseif ID == 15 then
		...
		if IsValid( target ) then
			...
			local tabRequest = Admin:GenerateRequest( "Enter reason for kick", "Kick player", "Kicked for being AFK.", ID )
			Core:Send( ply, "Admin", { "Request", tabRequest } )
		else
			Core:Send( ply, "Print", { "Admin", "Couldn't find a valid player with Steam ID: " .. Steam } )
		end
	...
	elseif ID == 21 then
		local tabRequest = Admin:GenerateRequest( "Enter the bonus points.", "Bonus multiplier", tostring( Timer.BonusMultiplier ), ID )
		Core:Send( ply, "Admin", { "Request", tabRequest } )
	elseif ID == 22 then
...
function Admin:HandleRequest( ply, args )
	...
	elseif ID == 5 then
		Core:Unload()
		  ...
		  -- Add a default prefix if the map name doesn't contain it
		  if not string.find(Value, "bhop_") then
			Value = "bhop_" .. Value
		end
	...
	elseif ID == 170 then
		...
		MySQL:Start( "DELETE FROM game_times WHERE szMap = '" .. game.GetMap() .. "' AND nStyle = " .. nStyle .. " AND szUID = '" .. szUID .. "'" )
		Timer:LoadRecords()
	...
	elseif ID == 22 then
		if not RTV:MapExists( Value ) then
			...
			MySQL:Start( "DELETE FROM game_times WHERE szMap = '" .. Value .. "'" )
	...
	elseif ID == 28 then
		...
		MySQL:Start( "DELETE FROM game_times WHERE szMap = '" .. game.GetMap() .. "' AND nStyle = " .. nStyle )
		Timer:LoadRecords()
