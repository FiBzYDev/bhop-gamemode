...
function PLAYER:StartTimer(savetime)
    ...
    timer.Create(timerName, tickDuration, 0, function()
        ...
        if SysTime() - self.lastGroundTouch >= 0.5 then
...
function PLAYER:BonusStart()
    ...
    self.Tb = SysTime() + engine.TickInterval() / 0.1 -- Use SysTime for internal precision timing
    self.displayTb = CurTime() -- Use CurTime for display purposes
    self.lastGroundTouch = SysTime() -- Initialize the last ground touch time

    -- Start the tick-based timer
    local bonusTimerName = "PlayerBonusTimer_" .. self:SteamID()
    local tickDuration = 0.1  -- Adjust this value as needed
    local elapsedTicks = 0

    timer.Create(bonusTimerName, tickDuration, 0, function()
        -- Increment elapsed ticks
        elapsedTicks = elapsedTicks + 1
    ...
    -- Send timer start event to the player/entity
    Core:Send(self, "Timer", {"Start", self.displayTb}) -- Use displayTb for displaying the bonus timer

    Bot:TrimRecording(self)
    Spectator:PlayerRestart(self)
    SMgrAPI:ResetStatistics(self)
    Core:Send(player.GetAll(), "Scoreboard", {"bonus", self, self.displayTb}) -- Use displayTb for the scoreboard
end
...
function Timer:Finish(ply, nTime)
    ...
    if OldRecord ~= 0 and nTime >= OldRecord then
        ...
        function()
            ...
            MySQL:Start(
                ...
                function(Rank)
                    ...
                    if nID == 1 and (ply.Style == _C.Style.Normal or ply.Style == _C.Style.Bonus or ply.Style == _C.Style.Segment) and DISCORD_WR_WEBHOOK then
                        ...
                        local embedColor = (ply.Style == _C.Style.Normal and Color(0, 165, 255) or Color(255, 255, 0)) -- Blue for Normal, Yellow for Bonus and Segment
                            ...
                            embeds = {
                                {
                                    ...
                                    color = embedColor.r * 65536 + embedColor.g * 256 + embedColor.b, -- Convert Color to integer for Discord color representation
                                    timestamp = timestamp
                                }
                            },
                            attachments = {}
