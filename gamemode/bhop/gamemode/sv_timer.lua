local PLAYER, CT, CAPVEL, strafeavg, nPage = FindMetaTable "Player", CurTime, 280, 0, nil -- pG gamemode uses SysTime  note: i dont know if it makes a difference
Timer = {}
Timer.Multiplier = 1
...
function PLAYER:StartTimer(savetime)
    ...
    if not savetime then
        self.Tn = CurTime() + engine.TickInterval() / 0.1
        self.lastGroundTouch = CurTime() -- Initialize the last ground touch time
    end
    ...
    timer.Create(timerName, tickDuration, 0, function()
        ...
        if self:IsOnGround() then
            self.lastGroundTouch = CurTime()
        end

        -- Check if the player has been on the ground for half a second or more
        if CurTime() - self.lastGroundTouch >= 0.5 then
