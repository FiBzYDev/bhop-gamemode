...
local function ChangeMap(ply, args)
    ...
    if not ply:IsAdmin() then
        -- If the player is not an admin, return without taking any action
        return
    end

    -- Check if args is nil (no arguments provided)
    if not args then
        -- If no arguments are provided, inform the player about the correct syntax
        Core:Send(ply, "Print", { "Notification", Lang:Get("MapChangeSyntax") })
        return
    end

    -- Check if the argument (map name) is provided
    local targetMap = args[1]
    if not targetMap then
        -- If no map name is provided, inform the player about the correct syntax
        Core:Send(ply, "Print", { "Notification", Lang:Get("MapChangeSyntax") })
        return
    end
...
self:Register({ "change" }, function(ply, args)
	...
	self:Register( { "serverules", "rules" }, function( ply, args )
		Core:Send( ply, "Rules" )
	end )

	self:Register( { "serversettings", "movement" }, function( ply, args )
	...
	self:Register( { "strafetrainer", "jcstrainer" }, function( ply )
	...
	self:Register( "_invalid", function( ply, args )
	end )
end
...
function Command:Trigger( ply, szCommand, szText )
	...
	if not szFunc then szFunc = "_invalid" end
...
function Command:Possible( ply ) return true end
...
function Command.Nominate( ply, _, varArgs )
	...
	local mapName = varArgs[1]

    if string.sub(mapName, 1, 5) == "bhop_" then
        mapName = string.sub(mapName, 6)
    end
