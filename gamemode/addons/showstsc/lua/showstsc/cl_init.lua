...
local function OpenMaterialPicker(triggerType, cb)
	...
	matLab:SetText(text)
	matLab:Dock(FILL)
	matLab:SetTextColor(color_white)
	matLab:SetContentAlignment(6)
...
local function OpenConfigMenu()
	...
	function w:Paint(w, h)
        draw.RoundedBox(4, 0, 0, w, h, Color(42, 42, 42, 255))
    end
	...
	enabTrig:SetTextColor(color_white)
	enabTrig:Dock(TOP)
	...
	function w.UpdateCountTooltips()
		...
		for brushType = 1, BrushType.MAX do
			local count = g_brushesCount[brushType]
